...
/**
 * SVARSRUBRIKERNA, PÅ ETT STÄLLE.
 *
 * De satt inne i sendJson, alltså i node:http-lagret. Det höll så länge
 * det fanns en väg ut. Det gör det inte: på Vercel svarar
 * api/[...path].ts, och den skrev sina egna rubriker - eller rättare
 * sagt, den skrev inga.
 *
 * `cache-control: no-store` är inte kosmetika. Ärendedata i en
 * insolvensakt får aldrig ligga kvar i en webbläsarcache eller hos ett
 * mellanled; en cachead /v1/cases som visas för nästa besökare är exakt
 * det som radskyddet finns för att förhindra, kringgått utanför
 * databasen.
 *
 * En andra uppsättning rubriker i Vercel-filen hade drivit isär tyst.
 * Därför en definition, använd av båda.
 */
export const SVARSRUBRIKER: Record<string, string> = {
