 ...
 *
 * Sviten körs med DOCUMENTS_BUCKET satt (se server/tests/run.sh), annars hade
...
{
  const { laesForstaBytes, laesHuvud, taBortObjekt } = await import("../storage");
...
const { provaFil } = await import("../filtyper");
...
/* --- 9b. VERCEL-INGÅNGEN, KÖRD PÅ RIKTIGT ------------------------------- */

/*
 * ALLT OVANFÖR PRÖVAR handle(). PÅ VERCEL ANROPAR INGEN handle() DIREKT.
 *
 * Varje anrop i drift går genom api/[...path].ts: den bygger URL:en ur
 * req.url och host-rubriken, kapar /api-prefixet, prövar hastighetsgränsen
 * och lämnar över. Går något fel DÄR är hela sviten ovan grön medan
 * produkten är trasig - vilket är precis den sortens hål som en flytt
 * öppnar.
 *
 * Handlern körs därför här, med riktiga req/res-attrapper mot samma
 * databas som resten av sviten.
 */
{
  const { default: vercelHandler } = await import("../../api/[...path]");
  ...
  const anropa = async (
    ...
    const fangat: Fangat = { status: 0, body: null, headers: {} };
    const res = {
      setHeader: (namn: string, varde: string) => {
        fangat.headers[namn.toLowerCase()] = varde;
      },
      status(kod: number) {
        fangat.status = kod;
        return this;
    ...
    await vercelHandler(
      {
        method,
        url,
        headers: { host: "clearance.se", ...(extra.headers ?? {}) },
        body: extra.body,
  ...
  // Rewriten i vercel.json skickar /v1/health hit som /api/v1/health.
  const halsa = await anropa("GET", "/api/v1/health");
  check("Vercel-ingången svarar på /api/v1/health", halsa.status === 200, halsa);

  /*
   * OCH DEN SÄTTER SAMMA RUBRIKER SOM DEN EGNA SERVERN.
   *
   * res.json() sätter bara content-type. Utan no-store kan en akt hamna i
   * en cache utanför vår kontroll - och radskyddet i databasen hjälper
   * inte mot ett svar som redan lämnat den.
   */
  check("svaret får aldrig cachas", halsa.headers["cache-control"] === "no-store", halsa.headers);
  check("och nosniff sätts", halsa.headers["x-content-type-options"] === "nosniff", halsa.headers);
  check("och referrern läcker inte", halsa.headers["referrer-policy"] === "no-referrer", halsa.headers);

  // Och utan prefix - Vercel kan routa hit på båda formerna.
  const utanPrefix = await anropa("GET", "/v1/health");
  check("och på /v1/health", utanPrefix.status === 200, utanPrefix);

  /*
   * PREFIXKAPNINGEN FÅR INTE VARA GIRIG.
   *
   * En naiv replace(/^\/api/, "") hade gjort /apinyckel till /nyckel.
   * Regexpen har (?=\/|$) för att hindra det; här prövas att den gör det.
   */
  const okand = await anropa("GET", "/api/v1/finns-inte-alls");
  check("en okänd resurs ger 404, inte 500", okand.status === 404, okand);

  // En riktig inloggning genom hela vägen: routern, databasen, radskyddet.
  const inlogg = await anropa("POST", "/api/v1/auth/login", {
    headers: { "content-type": "application/json" },
    body: { email: "agnes@bolag-a.se", password: "hemligt-losen-agnes" },
  });
  check("inloggning fungerar genom Vercel-ingången", inlogg.status === 200, inlogg);
  const polett = (inlogg.body as { token?: string } | null)?.token ?? "";
  check("och lämnar ut en sessionspolett", polett.length > 20);

  // Polletten bär identiteten vidare - alltså sätts app.user_id.
  const jag = await anropa("GET", "/api/v1/auth/me", {
    headers: { authorization: `Bearer ${polett}` },
  });
  check("polettens identitet gäller i nästa anrop", jag.status === 200, jag);

  // Utan polett ska samma resurs neka. Radskyddet failar stängt.
  const utan = await anropa("GET", "/api/v1/auth/me");
  check("utan polett nekas samma resurs", utan.status === 401, utan);

  /*
   * HASTIGHETSGRÄNSEN GÄLLER PÅ DEN HÄR VÄGEN.
   *
   * Den låg i createApiServer, som Vercel aldrig anropar. Hade den blivit
   * kvar där vore /v1/auth/login obegränsat i drift medan sviten ovan var
   * grön. Inloggningens tak är 10 på fem minuter (LOGIN i rateLimit.ts).
   */
  await nollstallGranser();
  let sista = { status: 0 } as Fangat;
  for (let i = 0; i < 12; i++) {
    sista = await anropa("POST", "/api/v1/auth/login", {
      headers: { "content-type": "application/json", "x-forwarded-for": "203.0.113.9" },
      body: { email: "ingen@exempel.se", password: "fel" },
    });
  }
  check("gränsen slår till på Vercel-vägen", sista.status === 429, sista.status);
  check("och säger när man får försöka igen", sista.headers["retry-after"] !== undefined, sista.headers);
  await nollstallGranser();
}
...
/* --- 11. Rollgrinden: vägrar, och cachar inte sin vägran ---------------- */

/*
 * TVÅ PÅSTÅENDEN I server/db.ts SOM INGET PROV RÖRDE.
 *
 *  1. Grinden VÄGRAR när rollen kringgår radskyddet. Sviten ansluter som
 *     superanvändaren, så den vägran hade annars bara varit prövad genom
 *     att undantaget ALLOW_UNSAFE_DB_ROLE=1 är satt - alltså inte alls.
 *  2. Ett MISSLYCKANDE CACHAS INTE. Kommentaren lovar att en rättad
 *     DATABASE_URL fungerar vid nästa request, utan omdeploy. Ett löfte
 *     i en kommentar är en åsikt tills det körs.
 *
 * Sist i filen, efter att poolen ändå är stängd.
 */
{
  const undantaget = process.env.ALLOW_UNSAFE_DB_ROLE;

  delete process.env.ALLOW_UNSAFE_DB_ROLE;
  nollstallRollGrind();
  let vagrade = false;
  try {
    await withAnon(async (tx) => tx.query("select 1"));
  } catch (fel) {
    vagrade = /osäker databasroll/.test(fel instanceof Error ? fel.message : String(fel));
  }
  check("grinden vägrar när rollen kringgår radskyddet", vagrade);

  // Utan nollställning: samma vägran, ur cachen? Nej - grinden nollställer
  // sig själv i catch, så nästa försök prövar på nytt.
  process.env.ALLOW_UNSAFE_DB_ROLE = "1";
  let slapptIgenom = false;
  try {
    await withAnon(async (tx) => tx.query("select 1"));
    slapptIgenom = true;
  } catch {
  ...
  check("och en rättad miljö fungerar direkt - misslyckandet cachas inte", slapptIgenom);

  if (undantaget === undefined) delete process.env.ALLOW_UNSAFE_DB_ROLE;
  else process.env.ALLOW_UNSAFE_DB_ROLE = undantaget;
  await closePool();
}
