...
check("det fria läget beskrivs", /Gratis tills du behöver mer/i.test(body));
...
await page.waitForTimeout(1500);
body = await page.innerText("body");
check("driftpanelen visar företagsabonnemanget", /Företagsabonnemanget/i.test(body) && /985 kr\/mån \+ moms/.test(body));
await page.fill('input[aria-label="Ny månadsavgift exklusive moms"]', "1200");
await page.click('button:has-text("Spara")');
await page.waitForTimeout(1000);
body = await page.innerText("body");
check("nytt belopp sparas", /1200 kr\/mån \+ moms/.test(body));
