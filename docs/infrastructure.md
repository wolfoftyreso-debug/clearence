...
| **API:et: identitet, sessioner, översiktens data och dess skrivvägar** | **Byggt.** `api/server/`, node:http med **ett** beroende (`pg`). Läsning och skrivning för ärenden, journal, beslut, uppgifter, betalningar, dokumentmetadata (inkl. signerad nedladdning), meddelanden, KBR, kontaktinkorgen och ärendets deltagare/inbjudningar. `npm run test:api` kör 170 kontroller mot riktig Postgres — identiteten läcker inte mellan samtidiga requests, en utomstående får TOMT på varje resurs, avbockningens tidpunkt sätts av servern, företagaren kan inte godkänna sitt eget underlag, en icke-administratör ser en tom kontaktinkorg, och en inbjudan kan bara accepteras av rätt adress |
| **API:et: resten av `DataPort`** | **Delvis.** Fakturering, katalog, drift, guidernas skrivningar — 102 metoder kvar. Samma mönster igen; siffran mäts av `test:awsadapter` och ska falla |
| **`awsAdapter` i klienten** | **Halvfärdig, och säger det själv.** `src/data/aws/`, vald med `VITE_DATA_ADAPTER=aws`. **37 av 139 portmetoder** går mot eget API; resten delegeras öppet till supabase-adaptern (strangler). `MIGRATED_PORTS` är listan och `npm run test:awsadapter` läser den — den som flyttar en port men glömmer listan får rött, och den som listar något oflyttat likaså. Delegeringen tas bort när listan täcker hela `DataPort` |
