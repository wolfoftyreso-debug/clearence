...
| P0-5 | Säkerhetsinvarianter verifierade i skarp miljö (RLS, roller, signerade URL:er, SHA-256-nycklar) | [KOD] | 🟡 | Byggt och testat i demo/self-hosted; ska köras om i produktionsmiljön. RLS-tester: `db/rls-tests`, `db/tests`. |
...
| Databasen | `bash db/tests/run.sh` och `bash db/rls-tests/run.sh` | Radskyddet, rollerna, jobben och raderingen - i BÅDA miljöerna. En skillnad mellan dem ska synas här, inte i produktion. Det självhostade lagret är det som gäller i drift: Vercel Postgres och Neon ÄR vanlig Postgres. |
